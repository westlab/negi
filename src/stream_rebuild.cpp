...
void StreamRebuild::SearchStream(Packet * pkt) {
    ...
    if (pkt->GetSyn() && !pkt->GetAck()) {
        ...
        if (tcp_conn_num == 0) {
            ...
            multimap<unsigned int, TcpConn *>::iterator tcp_conn_map_it =
            tcp_conn_pool->FindTcpConn(port_map_key);
            for (u_int i=0; i < tcp_conn_num; i++, tcp_conn_map_it++) {
                ...
                if (pkt->GetSrcPort() == tcp_conn->GetSrcPort() &&
                    pkt->GetDstPort() == tcp_conn->GetDstPort()) {
